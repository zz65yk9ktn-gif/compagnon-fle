...
"""
        ...
        try:
            ...
            status, _, page = request(port, "GET", start, cookie=cookie)
            assert status == 200 and "Soutien · niveau commun" in page
            from support_exams import SUPPORT_EXAMS_QUESTIONS
            for index in range(80):
                question_id = value(page, "question_id")
                run_id = value(page, "run_id")
                csrf = value(page, "csrf_token")
                question = next(item for item in SUPPORT_EXAMS_QUESTIONS if item["id"] == question_id)
                status, _, feedback = request(port, "POST", "/espace-apprenant/sequence-soutien-epreuves/demarrer", {
                    "csrf_token": csrf, "run_id": run_id, "question_id": question_id,
                    "answer": question["correct_answer"],
                }, cookie)
                assert status == 200 and "Bravo, c’est juste" in feedback
            ...
            status, _, page = request(port, "GET", start, cookie=cookie)
            assert status == 200 and "Soutien · niveau commun" in page
            from support_oral import SUPPORT_ORAL_QUESTIONS
            for index in range(40):
                question_id = value(page, "question_id")
                run_id = value(page, "run_id")
                csrf = value(page, "csrf_token")
                question = next(item for item in SUPPORT_ORAL_QUESTIONS if item["id"] == question_id)
                status, _, feedback = request(port, "POST", "/espace-apprenant/sequence-soutien-oral/demarrer", {
                    "csrf_token": csrf, "run_id": run_id, "question_id": question_id,
                    "answer": question["correct_answer"],
                }, cookie)
                assert status == 200 and "Bravo, c’est juste" in feedback
            ...
            status, _, page = request(port, "GET", start, cookie=cookie)
            assert status == 200 and "Soutien · niveau commun" in page
            from support_correction import SUPPORT_CORRECTION_QUESTIONS
            for index in range(64):
                question_id = value(page, "question_id")
                run_id = value(page, "run_id")
                csrf = value(page, "csrf_token")
                question = next(item for item in SUPPORT_CORRECTION_QUESTIONS if item["id"] == question_id)
                status, _, feedback = request(port, "POST", "/espace-apprenant/sequence-soutien-correction/demarrer", {
                    "csrf_token": csrf, "run_id": run_id, "question_id": question_id,
                    "answer": question["correct_answer"],
                }, cookie)
                assert status == 200 and "Bravo, c’est juste" in feedback
            ...
            status, _, page = request(port, "GET", start, cookie=cookie)
            assert status == 200 and "Soutien · niveau commun" in page
            from support_writing import SUPPORT_WRITING_QUESTIONS
            for index in range(64):
                question_id = value(page, "question_id")
                run_id = value(page, "run_id")
                csrf = value(page, "csrf_token")
                question = next(item for item in SUPPORT_WRITING_QUESTIONS if item["id"] == question_id)
                status, _, feedback = request(port, "POST", "/espace-apprenant/sequence-soutien-ecriture/demarrer", {
                    "csrf_token": csrf, "run_id": run_id, "question_id": question_id,
                    "answer": question["correct_answer"],
                }, cookie)
                assert status == 200 and "Bravo, c’est juste" in feedback
    ...
    print("SUPPORT_CHECK_OK (536 questions)")
