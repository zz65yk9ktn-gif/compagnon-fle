    ...
    with connect() as database:
        cursor = database.execute(
            ...
            INSERT INTO exercise_attempts
                (learner_id, sequence_slug, exercise_id, level, answer_text, is_correct, score)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (
                learner_id,
    ...
    with connect() as database:
        return database.execute(
            ...
            SELECT id, learner_id, sequence_slug, exercise_id, level,
                   answer_text, is_correct, score, attempted_at
    ...
    with connect() as database:
        summary = database.execute(
            ...
            SELECT
                ...
                COALESCE(SUM(is_correct), 0) AS success_count
            FROM exercise_attempts
            WHERE learner_id = ?
            """,
            (learner_id,),
